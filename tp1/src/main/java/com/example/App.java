...
public class App {
    ...
    private static void modifierProduit(EntityManagerFactory emf) {
        ...
        // 1. Chercher produit avec ID=3
        Produit p = em.find(Produit.class, 3L);

        // 2. Modifier son prix
        p.setPrix(new BigDecimal("3000.00"));

        // 3. Valider la transaction
        em.getTransaction().commit();
        em.close();

        System.out.println(" Le produit est modifié avec succès !");
    }
    private static void supprimerProduit(EntityManagerFactory emf) {
        ...
        System.out.println("  Le produit est supprimé avec succès !");
    }
